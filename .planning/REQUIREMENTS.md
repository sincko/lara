...
- [ ] **FNDT-01**: Repo has a single package manager (yarn) — package-lock.json removed, yarn.lock is the only lockfile
...
- [ ] **UPGR-01**: Gatsby upgraded to 5.16.1 (latest stable) with all gatsby-* plugins in lockstep (one commit, exact version matrix)
...
- [ ] **UPGR-07**: First post-upgrade Netlify deploy runs with cleared cache

### Form

- [ ] **FORM-01**: MUI v4 removed (core + icons) — contact form uses plain CSS with existing SCSS theme variables
