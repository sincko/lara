...
jest.mock("gatsby-image", () => ({ __esModule: true, Img: () => <img alt="" /> }))
...
jest.mock("@reach/router", () => ({ useLocation: () => ({ pathname: "/blog" }) }))
...
const renderBlog = pageContext =>
  render(
    <BlogIndex data={{ allMarkdownRemark: { edges: [] } }} pageContext={pageContext} />
  )
...
describe("Pagination", () => {
  it("page 1 of 3: no Previous link, Next points to /blog/2, number 1 is active", () => {
    ...
    expect(pagination.getByText("Next").closest("a")).toHaveAttribute("href", "/blog/2")
  ...
  it("page 2 of 3: Previous points to /blog/ (prevPage special case), Next to /blog/3, number 2 is active", () => {
    ...
    expect(pagination.getByText("Previous").closest("a")).toHaveAttribute("href", "/blog/")
    // nextPage = "/blog/" + (2 + 1) = "/blog/3"
    expect(pagination.getByText("Next").closest("a")).toHaveAttribute("href", "/blog/3")
  ...
  it("page 3 of 3: Previous points to /blog/2, no Next link (isLast), number 3 is active", () => {
    ...
    expect(pagination.getByText("Previous").closest("a")).toHaveAttribute("href", "/blog/2")
